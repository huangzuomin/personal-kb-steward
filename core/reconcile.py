...
只根据给定来源与既有页面，提出专题资料综合，不决定文件路径、对象 ID 或 revision。
输出一个 JSON 对象：decision、reason、summary、conflicts。
decision 只能是 create、update、conflict、noop。已有目标只能 update/noop/conflict，无目标只能 create/noop/conflict。
新资料没有值得加入的知识时选 noop。资料与现有事实存在无法消解的冲突时选 conflict，在 conflicts 字符串列表中说明，禁止擅自取舍。
create/update 的 summary 是完整的新版资料综合 Markdown，不是整页文件。综合旧来源和新增来源，不能丢掉仍然有效的内容。
每个实质段落必须用 [[完整来源相对路径.md]] 标明依据，只能引用 sources 中的文件；不要输出 frontmatter、HTML 注释或一级标题。
区分事实、解释和待验证判断，不把推测写成事实，不声称已自动核实。reason 解释本次决定。没有冲突时 conflicts=[]。
系统只替换专用资料综合区块，页面其余手写内容保持不变。不要试图修改其余内容；必须修改它们才能消解矛盾时返回 conflict。
"""
...
def _managed(text: str, metadata: dict[str, Any]) -> tuple[tuple[int, int] | None, dict[str, Any]]:
    ...
    if count == (0, 0) and state is None:
        return None, {}
    if count != (1, 1) or not isinstance(state, dict) or state.get("version") != 1:
...
def make_reconcile_plan(cfg: dict[str, Any], topic: str, sources: list[str], *,
    ...
    plan: dict[str, Any] = {
        ...
        "reconcile": {"version": 1, "decision": "conflict", "topic": topic, "target": target},
    }
    info = plan["reconcile"]
    try:
        ...
        if span and fingerprints == old_state.get("source_hashes"):
        ...
        if not isinstance(summary, str) or not summary.strip() or len(summary) > limit or "<!--" in summary:
            raise ReconcileConflict("模型综合内容为空、过长或包含区块控制标记")
        links = _LINK.findall(summary)
        if not links or any(link not in fingerprints for link in links):
            raise ReconcileConflict("综合内容必须使用提供的真实来源路径，不能生成未提供的双链")
        for paragraph in re.split(r"\n\s*\n", summary.strip()):
            if any(line.strip() and not line.lstrip().startswith("#") for line in paragraph.splitlines()) and not _LINK.search(paragraph):
                raise ReconcileConflict("综合内容存在未标注来源的段落，请重新生成")
        newline = "\r\n" if "\r\n" in original else "\n"
        summary = summary.replace("\r\n", "\n").strip().replace("\n", newline)
        section = newline.join([START, "## 资料综合", "", summary, END])
        state = {"version": 1, "source_hashes": fingerprints, "section_sha256": sha256_text(section)}
        if existing:
        ...
        plan["planned_pages"] = [{**(update_base(existing) if existing else {}),
            "skill": SKILL, "operation": expected, "rel_path": rel, "target": rel,
        ...
        return plan
    except (ReconcileConflict, ObjectIdentityError, UnicodeError, json.JSONDecodeError) as exc:
...
def validate_reconcile_page(index: VaultIndex, page: dict[str, Any]) -> None:
    ...
    for rel, expected in hashes.items():
        if _read(index, rel).sha256 != expected:
